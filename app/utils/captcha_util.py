...
def _build_svg(text: str, width: int = 120, height: int = 40) -> str:
    """构造包含验证码文本的 SVG 图片"""
    noise_lines = []
    for _ in range(3):
        # 随机生成干扰线段
...
async def create_captcha() -> CaptchaOut:
    """
    生成验证码
    Returns:
        {
    ...
    async with get_redis() as redis:
        await redis.setex(key, EXPIRE_SECONDS, code.lower())
