...
class Promotion(Base, TimestampMixin):
    ...
    discount_type: Mapped[str] = mapped_column(
        String(16), nullable=False, comment="优惠类型"
    )
    discount_value: Mapped[float] = mapped_column(
        Numeric(12, 2), nullable=False, comment="优惠值"
    )
    ...
    status: Mapped[str] = mapped_column(
        String(16), nullable=False, server_default=text("'active'"), comment="活动状态"
    )
    __table_args__ = (
